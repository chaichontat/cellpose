...
def _resolve_pack_stripe_height(
    ...
    *,
    ...
    The system auto-selects K to maximize packing while ensuring guard >= 5.
    ...
    candidate = (
        stripe_height
        if stripe_height is not None and stripe_height > 0
        else (max_h if max_h is not None else bsize)
    )
    ...
    # Try reducing the stripe height until packing works
    div = 16
    for attempt_h in range(candidate - div, div - 1, -div):
        if attempt_h <= 0:
            break
        if max_h is not None and attempt_h > max_h:
            continue
        layout = compute_stripe_layout(attempt_h, bsize=bsize, border=border)
        if layout is not None:
            return attempt_h, True
...
def _pack_training_tiles(
    ...
    *,
    ...
    """Pack cropped stripes into a single `bsize×bsize` tile to mirror inference packing.
    ...
    stripe_height : int | None
        ...
        violates the packing constraint), the height is auto-chosen.
    border : int
    ...
    N, C, H, W = imgs.shape
    desired_height = (
        stripe_height if (stripe_height is not None and stripe_height > 0) else H
    )
    ...
    if stripe_h is None or stripe_h <= 0:
        train_logger.debug(
            "Packing requested but layout unavailable (stripe_height=%s, bsize=%d); "
    ...
    if layout is None:
        train_logger.debug(
            "Resolved stripe height %d still cannot satisfy packing (bsize=%d); "
            "returning original batch of %d tiles",
            stripe_h,
            bsize,
            N,
    ...
    if auto_adjusted:
        train_logger.debug(
            "Auto-adjusted training stripe height to %d (K=%d, guard=%d, bsize=%d)",
            stripe_h,
            layout.K,
            layout.guard,
            bsize,
    ...
    if not _PACK_TRAIN_LOGGED:
        ...
        train_logger.info(
            "Packing training stripes: tiles=%d K=%d guard=%d stripe_height=%d -> groups=%d",
            N,
            layout.K,
            layout.guard,
            stripe_h,
    ...
    train_logger.debug(
        "Packing training batch: tiles=%d -> groups=%d (K=%d, guard=%d, stripe_height=%d, bsize=%d)",
        N,
    ...
    img_stripes = np.transpose(
        np.stack(img_stripes, axis=0), (0, 2, 3, 1)
    )  # [N, stripe_h, W, C]
    lbl_stripes = np.transpose(
        np.stack(lbl_stripes, axis=0), (0, 2, 3, 1)
    )  # [N, stripe_h, W, L]
    ...
    return imgs_out.astype(imgs.dtype, copy=False), lbls_out.astype(
        lbls.dtype, copy=False
    )
...
def _compute_min_effective_height(
    train_data: list,
    diam_train: np.ndarray,
    diam_mean: float,
) -> float:
    """Compute minimum effective height across loaded training images.

    Effective height = raw_height / (diam / diam_mean) = raw_height * diam_mean / diam
    This accounts for diameter-based rescaling during training.
    """
    min_eff_h = float("inf")

    for i, img in enumerate(train_data):
        raw_h = float(_prepad_height(img))
        # rescale factor = diam / diam_mean
        rsc = diam_train[i] / diam_mean
        eff_h = raw_h / rsc if rsc > 0 else raw_h
        if eff_h < min_eff_h:
            min_eff_h = eff_h

    return min_eff_h if min_eff_h != float("inf") else 68.0  # fallback to default
...
def _partition_thin_indices(
    indices: list[int], ratio: float
...
def _prepare_dimension_packed_batch(
    ...
    *,
    ...
    K is auto-selected by compute_stripe_layout to maximize packing while ensuring guard >= 5.
    """
    ...
    if not pack_enabled or pack_height is None:
    ...
    square_indices, large_indices = _partition_indices(
        large_indices, _SQUARE_STRIPE_RATIO
    )
    stripe_imgs: list[np.ndarray] = []
    stripe_lbls: list[np.ndarray] | None = [] if lbls is not None else None
    ...
    if len(thin_indices) > 0 or stripe_imgs:
        ...
        rsc_thin = (
            np.asarray(thin_rescales, dtype=rsc.dtype)
            if thin_rescales
            else np.zeros((0,), dtype=rsc.dtype)
        )
    ...
    if batches:
        ...
        lbl = (
            ...
            if len(labels_batches) > 1
            else labels_batches[0]
        )
...
class _ScaledTileSaver:
    ...
    def save_batch(
        self, tiles: np.ndarray, *, phase: str, epoch: int, batch_index: int
    ) -> None:
...
def _process_train_test(
    ...
    channel_axis=None,
    device=None,
    diameter_override: float | Sequence[float] | None = None,
):
    """
    ...
    # keep copies of raw mask labels for diameter estimation before flow conversion
    ...
    def _prepare_override_array(
        override: float | Sequence[float], count: int, split_name: str
    ) -> np.ndarray:
        if np.isscalar(override):
            arr = np.full(count, float(override), dtype=np.float32)
        else:
            if len(override) != count:
                raise ValueError(
                    f"diameter_override length mismatch for {split_name} split: expected {count}, got {len(override)}"
                )
            arr = np.asarray(override, dtype=np.float32)
        arr[arr < 5.0] = 5.0
        return arr

    def _count_masks(lbl: np.ndarray | None) -> int:
        """Count instance labels using only the mask channel.

        Labels may already include extra flow/probability channels; relying on
        np.unique over the full stack overcounts because flows are float-valued.
        We isolate the mask plane to mirror the inference branch that looks at
        labels[0].
        """
        if lbl is None:
            return 0
        arr = np.asarray(lbl)
        if arr.ndim >= 3:
            # Channel-first: mask channel is first plane
            arr = arr[0]
        uniques = np.unique(arr)
        return int((uniques > 0).sum())
    ...
    if diameter_override is None:
        ...
        for k in trange(nimg):
            tl = (
                raw_train_labels[k]
                if raw_train_labels is not None
        ...
        if test_data is not None and raw_test_labels is not None:
            diam_test = np.array(
                [
                    utils.diameters(raw_test_labels[k])[0]
                    for k in trange(len(raw_test_labels))
                ]
            )
        ...
        train_logger.info(
            ">>> using provided diameter_override; skipping diameter inference"
        )
        diam_train = _prepare_override_array(diameter_override, nimg, "train")
        for k in trange(nimg):
            tl = (
                raw_train_labels[k]
                if raw_train_labels is not None
            ...
            nmasks[k] = _count_masks(tl)

        if test_data is not None:
            n_test = len(test_labels) if test_labels is not None else len(test_data)
            diam_test = _prepare_override_array(diameter_override, n_test, "test")
        elif test_labels_files is not None:
            diam_test = _prepare_override_array(
                diameter_override, len(test_labels_files), "test"
            )
        else:
            diam_test = None
    ...
    if train_data is not None:
        train_data = _reshape_norm(
            train_data, channel_axis=channel_axis, normalize_params=normalize_params
        )
        normed = True
    if test_data is not None:
        test_data = _reshape_norm(
            test_data, channel_axis=channel_axis, normalize_params=normalize_params
        )
...
def train_seg(
    ...
    Args:
        ...
        diameter_override (float or Sequence[float], optional): When set, skip diameter inference and use this value
            (or per-image array) for train/test splits when computing rescaling factors. Values are clamped to >=5px.
        ...
        pack_to_single_tile (bool, optional): When True, crops stripes and packs them
            into a single `bsize×bsize` tile with guard rows so that training matches packed
            inference. K is auto-selected to maximize packing while ensuring guard >= 5.
            Defaults to False.
        ...
        pack_stripe_height (int or None, optional): Stripe height (in pixels) cropped from each training
            tile before packing. Defaults to None, which uses the smallest effective training image height
            across the dataset. Provide an explicit integer to override. Images with height < 100px are
            automatically routed to the packed path; others use standard training with full augmentation.
        ...
        iou_every (int or None, optional): If set, compute mean AJI (IoU-like) on the test split
            every `iou_every` epochs using inference-style mask reconstruction. Defaults to 10.
    ...
    # Default to moderate random scale jitter when not explicitly specified.
    scale_range = 0.4 if scale_range is None else scale_range

    # Optional IoU cadence (eval-only)
    if iou_every is not None and iou_every < 1:
        train_logger.warning("iou_every must be >= 1; disabling IoU evaluation")
        iou_every = None
    ...
    net.diam_labels.data = torch.Tensor([diam_mean]).to(device)
    print(f"set net.diam_labels to {net.diam_labels.data.item():0.2f}")

    # Pack setup: compute K based on min effective height of training images
    ...
    if pack_to_single_tile:
        ...
        # Use min effective height as stripe height (user can override via pack_stripe_height)
        ...
        if pack_height_effective is None:
            train_logger.warning(
                "Packing requested but no admissible stripe height fits within bsize=%d; disabling packing.",
                bsize,
            )
            pack_to_single_tile = False
        else:
            ...
            if layout is None:
                ...
                if auto_adjusted:
                    train_logger.info(
                        "Auto-adjusted training stripe height to %d (border=%d, K=%d, guard=%d, bsize=%d)",
                        pack_height_effective,
                        pack_border_effective,
                        pack_k_effective,
                        pack_guard_effective,
    ...
    try:
        with open(report_path, "w", encoding="utf-8") as rf:
            start_payload = {
                ...
                "cosine_hold_epochs": None
                if cosine_hold_epochs is None
                else int(cosine_hold_epochs),
                "cosine_min_lr": None
                if cosine_min_lr is None
                else float(cosine_min_lr),
    ...
    for iepoch in range(n_epochs):
        compute_iou_epoch = iou_every is not None and ((iepoch + 1) % iou_every == 0)
        masks_true_eval: list[np.ndarray] = []
        masks_pred_eval: list[np.ndarray] = []
        ...
        for k in range(0, nimg_per_epoch, batch_size):
            ...
            lavg += train_loss
            nsum += len(imgi)
            total_train_tiles += len(imgi)
            # per epoch training loss (sum over tiles)
            train_losses[iepoch] += train_loss
        if total_train_tiles > 0:
            train_losses[iepoch] /= total_train_tiles
        else:
            train_losses[iepoch] = 0.0
        if (
            pack_to_single_tile
        ...
        if True:
            ...
            if test_data is not None or test_files is not None:
                ...
                for ibatch in range(0, len(rperm), batch_size):
                    with torch.no_grad():
                        ...
                        test_loss *= len(imgi)
                        lavgt += test_loss
                        total_eval_tiles += len(imgi)

                        if compute_iou_epoch:
                            # numpy doesn't support bfloat16; cast on CPU for metric-only path
                            y_np = (
                                y.detach().to(torch.float32, copy=False).cpu().numpy()
                            )
                            lbl_np = lbl.to(torch.float32, copy=False).cpu().numpy()
                            for ii in range(y_np.shape[0]):
                                dP_pred = y_np[ii, -2:]
                                cellprob_pred = y_np[ii, -1]
                                mask_pred = dynamics.resize_and_compute_masks(
                                    dP_pred,
                                    cellprob_pred,
                                ...
                                mask_true = lbl_np[ii, 0].astype(np.int32)
                                masks_true_eval.append(mask_true)
                                masks_pred_eval.append(mask_pred.astype(np.int32))
                if total_eval_tiles > 0:
                    lavgt /= total_eval_tiles
                else:
                    lavgt = 0.0
                if (
                    pack_to_single_tile
                ...
                if compute_iou_epoch and masks_true_eval:
                    aji_vals = metrics.aggregated_jaccard_index(
                        masks_true_eval, masks_pred_eval
                    )
                    aji_vals = np.nan_to_num(aji_vals, nan=0.0, posinf=0.0, neginf=0.0)
                    aji_mean = float(np.mean(aji_vals))
                    train_logger.info(
                        "Epoch %d: AJI (test) mean=%.4f over %d samples",
                        iepoch + 1,
                        aji_mean,
                        len(masks_true_eval),
                    )
                    try:
                        ...
                        train_logger.warning(
                            f"could not append AJI metric to report: {_e}"
                        )
            ...
            try:
                ...
                train_logger.warning(
                    f"could not update training report at {report_path}: {_e}"
                )
            lavg, nsum = 0, 0
        ...
        with open(report_path, "a", encoding="utf-8") as rf:
            rf.write(
                json.dumps(
                    {
                        "type": "end",
                        "ended_at": datetime.datetime.now().strftime(
                            "%Y-%m-%dT%H:%M:%S"
                        ),
